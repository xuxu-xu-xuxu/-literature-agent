    ...
    <div className="h-full flex flex-col">
      ...
      {synonyms.length > 0 && (
        <div className="px-4 py-2 border-b border-slate-800 bg-slate-900/50 shrink-0">
          <p className="text-xs text-slate-500 mb-1.5">已收敛的同义实体:</p>
          <div className="flex flex-wrap gap-1">
            {synonyms.map((s, i) => (
              <span key={i} className="text-xs px-2 py-0.5 rounded bg-emerald-900/30 text-emerald-400 border border-emerald-800/50">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!loading && entities.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            <p className="text-sm text-slate-500">暂无提取的实体</p>
        ...
        {Object.entries(grouped).map(([type, items]) => (
          <div key={type} className="border-b border-slate-800 last:border-b-0">
            <div className="px-4 py-2.5 bg-slate-900/30 flex items-center justify-between sticky top-0">
            ...
            {items.map((entity) => (
              <div key={entity.id}>
                <button
                  ...
                  className="w-full px-4 py-2.5 flex items-center justify-between hover:bg-slate-900/50 transition-colors text-left"
                >
                ...
                {expanded === entity.id && (
                  <div className="px-4 pb-3 pl-6">
                    <div className="bg-slate-900 rounded-lg p-3 border border-slate-800">
                      ...
                      {entity.source_span && (
                        <div className="text-xs text-slate-600 mt-1.5 pt-1.5 border-t border-slate-800">
