...
PROBLEM_PATTERNS: list[tuple[str, tuple[str, ...]]] = [
    ("interface instability", ("interfacial resistance", "interface resistance", "interface instability", "interphase")),
    ("air/moisture sensitivity", ("moisture sensitivity", "air stability", "moisture stability", "humidity", "h2s release")),
    ("lithium dendrite", ("dendrite", "short circuit", "lithium penetration")),
    ("low ionic conductivity", ("low ionic conductivity", "poor ionic conductivity", "conductivity remains low")),
    ("grain boundary resistance", ("grain boundary resistance", "grain-boundary resistance", "grain boundary")),
    ("densification/porosity", ("densification", "dense pellet", "porosity", "porous", "void")),
    ("electrochemical-window limitation", ("electrochemical window", "oxidative stability", "reduction stability")),
    ("mechanical brittleness", ("brittle", "brittleness", "fracture", "crack")),
    ("cathode compatibility", ("cathode compatibility", "high-voltage cathode", "cathode interface")),
    ("anode compatibility", ("lithium metal", "anode compatibility", "anode interface")),
]
...
async def load_topic_cards(
    ...
    async for db in get_db():
        ...
        if paper_ids:
            ...
            record_rows = (
                await db.execute(
                    select(SolidElectrolyteRecord).where(SolidElectrolyteRecord.paper_id.in_(paper_ids))
                )
            ).scalars().all()
            property_rows = (
                await db.execute(
                    select(SolidElectrolyteProperty).where(SolidElectrolyteProperty.paper_id.in_(paper_ids))
                )
            ).scalars().all()
        break
    ...
    records_by_paper: dict[str, list[SolidElectrolyteRecord]] = defaultdict(list)
    for row in record_rows:
        records_by_paper[row.paper_id].append(row)

    properties_by_paper: dict[str, list[SolidElectrolyteProperty]] = defaultdict(list)
    for row in property_rows:
        properties_by_paper[row.paper_id].append(row)
    ...
    for paper in papers:
        ...
        signal_map = _merge_signal_map(
            ...
            {
                "materials": [
                    _clean_signal_label(record.normalized_formula or record.material_formula)
                    for record in records_by_paper.get(paper.id, [])
                ],
                "methods": [
                    _clean_signal_label(record.method)
                    for record in records_by_paper.get(paper.id, [])
                ],
                "properties": ["ionic conductivity" for _ in records_by_paper.get(paper.id, [])],
            },
        )
        signal_map = _merge_signal_map(
            signal_map,
            {
                "materials": [
                    _clean_signal_label(prop.normalized_formula or prop.material_name)
                    for prop in properties_by_paper.get(paper.id, [])
                ],
                "methods": [_clean_signal_label(prop.method) for prop in properties_by_paper.get(paper.id, [])],
                "properties": [_clean_signal_label(prop.property_name) for prop in properties_by_paper.get(paper.id, [])],
            },
        )
